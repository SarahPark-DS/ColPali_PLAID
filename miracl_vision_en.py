...
def create_plaid_index(ps, device=DEVICE):
    ...
    # CPU로 index 생성: 대규모 corpus에서 GPU OOM 방지
    index.create(documents_embeddings=[d.cpu().to(torch.float32) for d in ps])
    return index

def get_topk_plaid(qs, plaid_index, k=10, batch_size=128, device=DEVICE):
    scores_list = []
...
print("\n[2] Fast PLAID...")
t0 = time.perf_counter()
plaid_index = create_plaid_index(image_embeddings, device=DEVICE)
t_build = time.perf_counter() - t0
print(f"  Index build time : {t_build:.2f}s")

t0 = time.perf_counter()
plaid_results_batched = get_topk_plaid(query_embeddings, plaid_index, k=10, device=DEVICE)
...
print(f"{'='*W}")
print(f"  {'Method':<26} {'R@1':>7} {'nDCG@5':>8} {'nDCG@10':>9} {'Img Emb(s)':>11} {'Search(s)':>10} {'Total(s)':>9}")
print(f"  {'-'*W}")
print(f"  {'Naive (score_multi_vector)':<26} {recall_naive:>6.2%} {ndcg5_naive:>8.4f} {ndcg10_naive:>9.4f} {t_img_emb:>11.1f} {t_naive:>10.2f} {t_img_emb+t_naive:>9.1f}")
print(f"  {'Fast PLAID (search only)':<26} {recall_plaid:>6.2%} {ndcg5_plaid:>8.4f} {ndcg10_plaid:>9.4f} {t_img_emb:>11.1f} {t_plaid:>10.2f} {t_img_emb+t_plaid:>9.1f}")
print(f"  {'Fast PLAID (build+search)':<26} {recall_plaid:>6.2%} {ndcg5_plaid:>8.4f} {ndcg10_plaid:>9.4f} {t_img_emb:>11.1f} {t_build+t_plaid:>10.2f} {t_img_emb+t_build+t_plaid:>9.1f}")
print(f"{'='*W}")
