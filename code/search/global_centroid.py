"""
Global Centroid Pruning (Level 1)
==================================
상한선 공식:
  cluster_bound[t, j] = ⟨q_t, c_j⟩ + ‖q_t‖ · r_j
  doc_bound[d]        = Σ_t  max_{j∈J(D)}  cluster_bound[t, j]

block_centroid.py (Level 2, Block Residual)보다 상한선이 느슨하지만
블록 분해 연산이 없어 계산 비용이 낮습니다.

인덱스는 block_centroid.load_cluster_index() 를 그대로 공유합니다.
(cluster_index_block.npz 의 radii 키: shape (K,))
"""
...
def global_centroid_search(
    ...
    radii            = idx_assets["radii"]            # (K,)  전역 반지름
    ...
    for q_idx in range(n):
        ...
        # Level 1 클러스터 상한선: ⟨q_t, c_j⟩ + ‖q_t‖ · r_j
        q_c_dot        = torch.matmul(query_emb, centroids.T)            # (m, K)
        q_norm         = torch.norm(query_emb, p=2, dim=1, keepdim=True) # (m, 1)
        cluster_bounds = q_c_dot + q_norm * radii.unsqueeze(0)           # (m, K)

        # 문서별 상한선: Σ_t max_{j∈J(D)} cluster_bounds[t, j]
        upper_bounds = []
